...
{
    public class ServiceBusTopicTrigger2
    {
        private readonly ILogger<ServiceBusTopicTrigger2> _logger;

        public ServiceBusTopicTrigger2(ILogger<ServiceBusTopicTrigger2> log)
        {
            _logger = log;
        }

        //[Disable] // when you want to disable a specific function inside your functionapp
        [FunctionName("ServiceBusTopicTrigger2")]
        public void Run([ServiceBusTrigger("labservicebustopic", "labServiceBusTopicSub2", Connection = "labsServiceBusNamespace_SERVICEBUS")] Message message, MessageReceiver messageReceiver)
        {
            ...
            _logger.LogInformation($"C# ServiceBus queue trigger function processed message topic 2: {message.Body.ToString()}");
