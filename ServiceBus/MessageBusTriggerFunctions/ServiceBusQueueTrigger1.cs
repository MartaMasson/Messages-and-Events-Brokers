...
namespace labsservicebusfunctionapp
{
    public class ServiceBusQueueTrigger1
    {
        ...
        public void Run([ServiceBusTrigger("labservicebusqueue", Connection = "labsServiceBusNamespace_SERVICEBUS")] Message message, ILogger log,  MessageReceiver messageReceiver)
        {
            ...
            log.LogInformation($"C# ServiceBus queue trigger function processed message from Queue1: {message.Body.ToString()}");
            log.LogInformation($"C# Message: {message.ToString()}, Delivery Count: {message.SystemProperties.DeliveryCount.ToString()}, LockToken: {message.SystemProperties.LockToken.ToString()}");
